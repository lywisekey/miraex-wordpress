class MfnForm {
	...
	values( obj, field, is_class = false ) {
		...
		if (obj.jsclass != 'themeoption' && _.has(field, 'responsive') && _.has(tmp_object, 'hotspots') && Array.isArray(tmp_object.hotspots) ) {

		  _.map(tmp_object.hotspots, (hotspot) => {
		    if (!_.has(hotspot, 'val')) return;
		    if (!_.has(hotspot.val, field.id)) return;

		    const hotspotField = hotspot.val[field.id];

		    if (!_.has(mfn.placeholders, field.id)) {
		      mfn.placeholders[field.id] = {};
		    }
		    ...
		    _.map(this.devices, (device) => {
		      if (
		        _.has(hotspotField, 'val') &&
		        _.has(hotspotField.val, device)
		      ) {
		        if (
		          !_.has(mfn.placeholders[field.id], device) ||
		          !Array.isArray(mfn.placeholders[field.id][device])
		        ) {
		          mfn.placeholders[field.id][device] = [];
		        }

		        mfn.placeholders[field.id][device].push({
		          val: hotspotField.val[device],
		          type: type,
		          screen: device,
		          hotspot: hotspot.hash,
		        });
		      }
		    });
		...
		}else if( obj.jsclass != 'themeoption' && _.has(field, 'responsive') ) {

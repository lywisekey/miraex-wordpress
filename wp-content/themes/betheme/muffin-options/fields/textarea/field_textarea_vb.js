  ...
  var MfnFieldTextarea = (function($) {
    ...
    function mfn_textarea_actions ( actionName ){
      ...
      switch( true ){
        ...
        case (actionName === 'lead'):
          __editor.methods.wrapTextIntoShortcode(`<p class="lead">`, `</p>`);
          break;
    ...
    function cmOn() {
        ...
        const settings = window.mfn_cm && window.mfn_cm.html 
            ? window.mfn_cm.html 
            : { codemirror: {} };
        ...
        settings.codemirror.lineWrapping = true; // <-- Rozwiązanie problemu z długą linią
        settings.codemirror.lineNumbers = false;
        settings.codemirror.autoRefresh = true;

        if ( $('#mfn-visualbuilder').hasClass('mfn-ui-dark') ) {
            settings.codemirror.theme = 'base16-dark';
        }

        __editor.instance = wp.codeEditor.initialize(__editor.domLocation, settings);
        ...
        if ( !$iframeCont.hasClass('mfn-watchChanges') ) {
            ...
            if ( _.has(inlineEditors, inlineIndex) ) {
                inlineEditors[inlineIndex].subscribe('editableInput', function(e, t) {
                    var cleanContent = $(t).html().replaceAll('&quot;', '');
                    edited_item.attr.content = cleanContent;
                    __editor.instance.codemirror.setValue( cleanContent );
                });
            }
        }
    ...
    function cmOff() {
      ...
      $('.mfn-visualbuilder .sidebar-panel-content .panel-edit-item .mfn-sc-editor').off();

      $('.mfn-visualbuilder .sidebar-panel-content .panel-edit-item .CodeMirror-wrap').remove();

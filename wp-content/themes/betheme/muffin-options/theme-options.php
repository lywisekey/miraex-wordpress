...
if( ! function_exists( 'mfna_pages' ) )
{
	...
	function mfna_pages() {
		...
		if( !is_admin() || empty($_GET['page']) ) return $array;

		// $pages = get_pages( 'sort_column=post_title&hierarchical=0' );

		$pages = get_posts([
			'post_type' => 'page',
	    'posts_per_page' => 1000, // limit to prevent Memory Exhausted on huge database
			'orderby' => 'title',
    	'order' => 'ASC',
		]);
		...
		// apply a custom filter to add more pages on 1000+ posts sites
		$array = apply_filters( 'mfna_pages', $array );
...
if( ! function_exists( 'mfna_header_checkout_get' ) )
{

	function mfna_header_checkout_get(){
		$array = [
			'0' => __( ' - Default - ', 'mfn_opts' ),
		];
		...
		$has_checkout_header = false;

		if( !empty( $templates ) ) {

			foreach($templates as $h){
				if( !empty( get_post_meta($h->ID, 'mfn-is-checkout-header', true) ) ){
					$has_checkout_header = true;
					break;
				}
			}

			if( !$has_checkout_header ) $array['create_checkout_header'] =  __( 'Checkout Header', 'mfn_opts' );
		}else{
			$array['create_checkout_header'] =  __( 'Checkout Header', 'mfn_opts' );
		}
...
if( ! function_exists( 'mfna_all_posts' ) ) {
	...
	function mfna_posts_list($search, $post_type = false) {
		...
		$array = array();
		$limit = (int) apply_filters( 'mfn_dynamic_data_hints_limit', 20 );
		...
		if( !$post_type ){
			...
			$array[$post_type] = array( 'label' => __( $post_type, 'mfn_opts' ) );
		}
		...
		foreach( $array as $key=>$value ) {
			...
			if( !empty($search) ) {
				...
				$array[$key]['options'] = $wpdb->get_results( "SELECT ID as id, post_title as title FROM {$wpdb->prefix}posts WHERE post_status = 'publish' AND post_type = '{$key}' LIMIT $limit" );
			}
...
if( ! function_exists( 'mfna_sidebars' ) )
{

	function mfna_sidebars($key_label = false) {

		$default = [
			'' => __('-- Default --', 'mfn-opts'),
		];

		// $default['#optgroup-custom'] = 'Custom';

		$theme_options = mfn_opts_get('sidebars', []);

		$array = array_merge( $default, $theme_options );

		// $array['#optgroup-custom-end'] = '';
		// $array['#optgroup-default'] = 'Default';

		foreach ( $GLOBALS['wp_registered_sidebars'] as $sidebar ) {
			if( 0 !== strpos($sidebar['id'], 'sidebar-') ){
				$array[$sidebar['id']] = ucwords( $sidebar['name'] );
			}
	 	}

		// $array['#optgroup-default-end'] = '';

		// visual builder - key label format to prevent default js array sorting
...
if( ! function_exists( 'mfn_opts_setup' ) )
{
	...
	function mfn_opts_setup(){
		...
		$sections['general'] = array(
			...
			'fields' => array(
				...
				array(
					'id' => 'section-padding',
					'type' => 'text',
					'title' => __('Section side padding', 'mfn-opts'),
					'desc' => __('Recommended for site width > 1240px to look good on smaller desktop screens. Exclude mobile.', 'mfn-opts'),
					'after' => 'px',
					'class' => 'narrow',
					'std' => 0,
					'responsive' => 'desktop',
		...
		$sections['buttons'] = array(
          ...
          'std' => '',
				...
				array(
					'id' => 'button-action-icon-color',
					'type' => 'color_multi',
					'data_attr' => 'data-csspath=".button.button_action i" data-responsive="desktop" data-style="color" data-unit=""',
					'title' => __('Icon color', 'mfn-opts'),
					'class' => 'form-content-full-width preview-color to-inline-style',
					'std' => [
						'normal' => '#626262',
		...
		$sections['advanced'] = array(
			...
			'fields' => array(
				...
				array(
					...
					'attr_id' => 'preloader',
					'type' => 'switch',
					'title' => __('Preloader', 'mfn-opts'),
					'desc' => __('Show the page when it\'s fully loaded', 'mfn-opts'),
					'options' => [
						'' => __('Hide', 'mfn-opts'),
						'load' => __('Show', 'mfn-opts'),
					],
				),
				...
				array(
					'id' => 'preloader-image',
					'type' => 'upload',
					'title' => __('Preloader image', 'mfn-opts'),
					'condition' => array( 'id' => 'preloader', 'opt' => 'is', 'val' => 'load' ),
				),
				...
				array(
					...
					'desc' => __( '<b>Legacy module, no longer supported</b>', 'mfn-opts' ),
					'options' => array(
				...
				array(
					...
					'options' => array(
						...
						'layout' => __('<span>Layouts - depreceted</span>', 'mfn-opts'),
		...
		$sections['blog'] = array(
			...
			'fields'	=> array(
				...
				array(
					'title' => __('Layout', 'mfn-opts'),
					'type' => 'header',
					'sub_desc' => __('<b>Image Sizes</b> used for specific <i>Layouts</i>, are listed under: <a href="#featured-image" target="_blank">Blog, Portfolio & Shop > Featured Image</a>', 'mfn-opts'),
				),
				...
				array(
					'id' => 'read-more-icon',
					'type' => 'icon',
					'title' => __('Read more Icon', 'mfn-opts'),
					'desc' => __('Enter # to hide icon', 'mfn-opts'),
					'std' => 'icon-doc-text',
				),
			...
			'fields'	=> array(
				...
				array(
					...
					'options' => array(
						'0' => __('Disable', 'mfn-opts'),
						'1' => __('Enable', 'mfn-opts'),
						'hide-price' => __('Enable and hide prices', 'mfn-opts'),
					),
					'std' => '0',
					'class' => 're_render_to',
					're_render_if' => 'div|div.default-woo-loop',
				...
				array(
					'id' => 'shop-wishlist-type',
					'attr_id' => 'shop-wishlist-type',
					'condition' => array( 'id' => 'shop-wishlist', 'opt' => 'is', 'val' => '1' ),
					'type' => 'switch',
					'title' => __('Single product wishlist', 'mfn-opts'),
					'desc' => __('Button works with templates only', 'mfn-opts'),
					'options' => array(
						'' => __('Icon', 'mfn-opts'),
				...
				array(
					'id' => 'shop-button-label-wishlist',
					'type' => 'text',
					'condition' => array('id' => 'shop-wishlist-type', 'opt' => 'is', 'val' => 'button'),
					'title' => __('Wishlist button label', 'mfn-opts'),
					'std' => 'Add to wishlist',
				),
				...
				array(
					...
					'join' => true,
					'type' => 'header',
					//'sub_desc' => __( 'The options refer to the block-based checkout. You can manage the legacy checkout using <a target="_blank" href="edit.php?post_type=template&tab=checkout">templates.</a>', 'mfn-opts' ),
				),
				...
				array(
					'id' => 'info-shopify',
					'type' => 'info',
					'class' => 'mfn-apply-shopify-checkout',
					'title' => __('Do you want checkout to look like Shopify’s? <a href="#">Apply recommended settings</a>', 'mfn-opts'),
				),

				array(
					'id' => 'gutenberg-checkout',
					'attr_id' => 'gutenberg-checkout',
					'type' => 'switch',
					'title' => __('Block-based checkout style', 'mfn-opts'),
					'desc' => __('Works with WooCommerce Checkout Block only', 'mfn-opts'),
					'options' => array(
						'' => __('Default', 'mfn-opts'),
						'1' => __('Summary background', 'mfn-opts'),
					),
					'std' => ''
				),

				array(
					'id' => 'gutenberg-checkout-summary-background',
					'type' => 'color',
					'title' => __('Checkout summary background', 'mfn-opts'),
					'alpha' => true,
					'condition' => array( 'id' => 'gutenberg-checkout', 'opt' => 'is', 'val' => '1' ),
				),

				array(
					'id' => 'gutenberg-checkout-header',
					'type' => 'select',
					'title' => __( 'Header', 'mfn-opts' ),
					'desc' => __( 'Click <a target="_blank" href="edit.php?post_type=template&tab=header">here</a> to manage headers.', 'mfn-opts' ),
					'options' => mfna_header_checkout_get(),
				),

				array(
					'id' => 'gutenberg-checkout-options',
					'type' => 'checkbox',
					'invert' => true,
					'title' => __('Visibility options', 'mfn-opts'),
					'options' => array(
						'hide-subheader'	=> __('Subheader', 'mfn-opts'),
						'hide-steps' 		=> __('Checkout steps', 'mfn-opts'),
						'hide-footer'		=> __('Footer', 'mfn-opts'),
					)
				),
			...
			'fields'	=> array(
				...
				array(
					...
					'options' => array(
						...
						'slider'	=> __( 'Gallery slider', 'mfn-opts' ),
		...
		$sections['shop-single'] = array(
			...
			'fields'	=> array(
				...
				array(
					'id' => 'shop-product-gallery-grid-cols',
					'type' => 'text',
					'title' => __('Columns', 'mfn-opts'),
					'desc' => __('Gallery grid columns count', 'mfn-opts'),
					'param' => 'number',
					'after' => 'cols',
					'class' => 'narrow mfn_field_desktop to-inline-style',
					'responsive' => 'desktop',
					'data_attr' => 'data-csspath=".column_product_images .mfn-product-images-wrapper .mfn-product-gallery-grid" data-responsive="desktop" data-style="column-count"',
				),

				array(
					'id' => 'shop-product-gallery-grid-cols-tablet',
					'type' => 'text',
					'title' => __('Columns', 'mfn-opts'),
					'desc' => __('Gallery grid columns count', 'mfn-opts'),
					'param' => 'number',
					...
					'responsive' => 'tablet',
					'data_attr' => 'data-csspath=".column_product_images .mfn-product-images-wrapper .mfn-product-gallery-grid" data-responsive="tablet" data-style="column-count"',
				),

				array(
					'id' => 'shop-product-gallery-grid-cols-mobile',
					'type' => 'text',
					'title' => __('Columns', 'mfn-opts'),
					'desc' => __('Gallery grid columns count', 'mfn-opts'),
					'param' => 'number',
					...
					'responsive' => 'mobile',
					'data_attr' => 'data-csspath=".column_product_images .mfn-product-images-wrapper .mfn-product-gallery-grid" data-responsive="mobile" data-style="column-count"',
				),
		...
		$sections['shop-addons'] = array(
			...
			'fields'	=> array(
				...
				array(
					...
					'desc' => __('Works in Shop sidebar and classic cart (non-Blocks). For Gutenberg/Blocks cart, please add it directly on the Cart page.', 'mfn-opts'),
					'options' => array(
						'0' => __('Disabled', 'mfn-opts'),
						'1' => __('Enabled', 'mfn-opts'),
					),
		...
		$sections['shop-addons-design'] = array(
			...
			'fields'	=> array(
				...
				array(
					'title' => __('Product gallery slider', 'mfn-opts'),
					'join' => true,
					'type' => 'header',
				),

				array(
					'id' => 'product-list-gallery-slider-nav-offset',
					'type' => 'text',
					'title' => __('Nav & Pagination offset', 'mfn-opts'),
					'class' => 'narrow',
					'param' => 'number',
					'after' => 'px',
					'std' => '',
				...
				array(
					'id' => 'product-list-gallery-slider-border-radius',
					'type' => 'text',
					'title' => __('Nav & Pagination border radius', 'mfn-opts'),
					'class' => 'narrow',
					'param' => 'number',
					'after' => 'px',
					'std' => '',
				...
				array(
					'id' => 'product-list-gallery-slider-arrows-visibility',
					'type' => 'switch',
				...
				array(
					'id' => 'product-list-gallery-slider-arrow-bg',
					'type' => 'color_multi',
					'title' => __('Arrow background', 'mfn-opts'),
					'data_attr' => 'data-csspath=".mfn-product-list-gallery-slider" data-responsive="desktop" data-style="--mfn-product-list-gallery-slider-arrow-bg" data-unit=""',
					'class' => 'form-content-full-width preview-color to-inline-style',
					'std' => [
						'normal' => '#fff',
						'hover' => '#fff',
				...
				array(
					'id' => 'product-list-gallery-slider-arrow-color',
					'type' => 'color_multi',
					'title' => __('Arrow color', 'mfn-opts'),
					'data_attr' => 'data-csspath=".mfn-product-list-gallery-slider" data-responsive="desktop" data-style="--mfn-product-list-gallery-slider-arrow-color" data-unit=""',
					'class' => 'form-content-full-width preview-color to-inline-style',
					'std' => [
						'normal' => '#000',
						'hover' => '#000',
				...
				array(
					'id' => 'product-list-gallery-slider-pagination-style',
					'type' => 'switch',
					'title' => __('Pagination style', 'mfn-opts'),
					'options' => array(
						'' => __('Dots', 'mfn-opts'),
				...
				array(
					'id' => 'product-list-gallery-slider-pagination-dots-size',
					'type' => 'text',
					'title' => __('Pagination dot size', 'mfn-opts'),
					'class' => 'narrow',
					'param' => 'number',
					'after' => 'px',
					'std' => '',
				...
				array(
					'id' => 'product-list-gallery-slider-pagination-dots-gap',
					'type' => 'text',
					'title' => __('Pagination dots gap', 'mfn-opts'),
					'class' => 'narrow',
					'param' => 'number',
					'after' => 'px',
					'std' => '',
				...
				array(
					'id' => 'product-list-gallery-slider-dots-bg',
					'type' => 'color_multi',
					'title' => __('Pagination color', 'mfn-opts'),
					'data_attr' => 'data-csspath=".mfn-product-list-gallery-slider" data-responsive="desktop" data-style="--mfn-product-list-gallery-slider-dots-bg" data-unit=""',
					'class' => 'form-content-full-width preview-color to-inline-style',
					'std' => [
				...
				array(
					'id' => 'product-list-gallery-slider-pagination-bg',
					'type' => 'color',
					'title' => __('Pagination background', 'mfn-opts'),
					'data_attr' => 'data-csspath=".mfn-product-list-gallery-slider" data-responsive="desktop" data-style="--mfn-product-list-gallery-slider-padination-bg" data-unit=""',
					'class' => 'form-content-full-width preview-color to-inline-style',
					'alpha' => 'true',
				),
				...
				array(
					'id' => 'sidecart-product-quantity-button-background',
					'type' => 'color',
					'alpha' => true,
					'title' => __('Product quantity button background', 'mfn-opts'),
					'condition' => array( 'id' => 'shop-sidecart', 'opt' => 'is', 'val' => '1' ),
				),

				array(
					'id' => 'sidecart-product-quantity-button-color',
					'type' => 'color',
					'alpha' => true,
					'title' => __('Product quantity button color', 'mfn-opts'),
					'condition' => array( 'id' => 'shop-sidecart', 'opt' => 'is', 'val' => '1' ),
				),
		...
		$sections['pages-under'] = array(
			...
			'fields'	=> array(
				...
				array(
					...
					'condition' => array( 'id' => 'construction-page', 'opt' => 'isnt', 'val' => '0' ), // is or isnt and value
					'type' => 'switch',
					'title' => __('Header', 'mfn-opts'),
					'options' => array(
						'0' => __('Hide', 'mfn-opts'),
		...
		$sections['search-form'] = array(
			...
			'fields' => array(
				...
				array(
					...
					'type' => 'switch',
					'title' => __('Search Mode', 'mfn-opts'),
					'condition' => array( 'id' => 'header-search', 'opt' => 'is', 'val' => 'shop' ), // is or isnt and value
					'options' => array(
						'' => __('Any words', 'mfn-opts'),
						'exact' => __('Exact phrase', 'mfn-opts'),
					),
					'std' => '',
				),
		...
		$sections['responsive'] = array(
			...
			'fields' => array(
				...
				array(
					'id' => 'tap-highlight-disable',
					'type' => 'switch',
					'title' => __('WebKit Tap Highlight', 'mfn-opts'),
					'desc' => __('Highlight that appears over a link while it\'s being tapped', 'mfn-opts'),
					'options' => array(
						'1' => __('Disable', 'mfn-opts'),
						'0' => __('Enable', 'mfn-opts'),
					),
		...
		$sections['addons'] = array(
			...
			'fields'	=> array(
				...
				array(
					...
					'title' => __( 'Options', 'mfn-opts' ),
					'options' => array(
						'disable' => __( 'Disable <strong>Magnific Popup</strong><span>Disable if you prefer to use external plugin</span>', 'mfn-opts' ),
						'disable-swiper' => __( 'Disable <strong>Swiper</strong><span>Disable if the addon is enqueued in your plugin, like Elementor</span>', 'mfn-opts' ),
						'disable-mobile' => __( 'Disable on <strong>mobile</strong> only', 'mfn-opts' ),
						'title' => __( 'Display image <strong>alt</strong> text as caption for lightbox image', 'mfn-opts' ),
					),
				),
		...
		$sections['colors-shop'] = array(
			...
			'fields' => array(
				...
				array(
					'title' => __('Category desc', 'mfn-opts'),
					'join' => true,
					'type' => 'header',
				),

				array(
					'id' => 'color-shop-cat-desc-gradient',
					'type' => 'color_multi',
					'title' => __('Category read more gradient', 'mfn-opts'),
					'class' => 'form-content-full-width re_render_to',
					'alpha' => true,
					'std' => [
		...
		$sections['translate-general'] = array(
			...
			'fields' => array(
				...
				array(
					'id' => 'translate-see-more',
					'type' => 'text',
					'title' => __('See more', 'mfn-opts'),
					'desc' => __('Filters toggle', 'mfn-opts'),
					'std' => 'See more',
				),

				array(
					'id' => 'translate-see-less',
					'type' => 'text',
					'title' => __('See less', 'mfn-opts'),
					'desc' => __('Filters toggle', 'mfn-opts'),
					'std' => 'See less',
				),
		...
		$sections['translate-shop'] = array(
			...
			'fields' => array(
				...
				array(
					'title' => __('My account', 'mfn-opts'),
					'type' => 'header',
					'join' => true,
				),

				array(
					'id' => 'translate-my-account-heading',
					'type' => 'text',
					'title' => __('My account heading', 'mfn-opts'),
					'desc' => __('Hello user_name in my account modal', 'mfn-opts'),
					'std' => 'Hello %s',
				),
    ...
    $sections['gdpr2-general'] = array(
      ...
      'fields' => array(
        ...
        array(
          'id' => 'gdpr2-button',
          'attr_id' => 'gdpr2-button',
          'type' => 'switch',
          'title' => __('Re-open modal button', 'mfn-opts'),
        ...
        array(
          ...
          'type' => 'switch',
          'title' => __('Close animation', 'mfn-opts'),
          'options' => array(
            '' => __('None', 'mfn-opts'),
            'fade' => __('Fade', 'mfn-opts'),
            'slide' => __('Slide', 'mfn-opts'),
          ),
          'desc' => __('Animation on popup close', 'mfn-opts'),
          'std' => 'fade',
          'condition' => array( 'id' => 'gdpr2', 'opt' => 'isnt', 'val' => '' ), // is or isnt and value
        ),
        ...
        array(
          ...
          'type' => 'text',
          'title' => __('Title', 'mfn-opts'),
          'desc' => __('Tab title<br />Use span with class hide-mobile to hide part of the text on mobile. Example: About Cookies -> About', 'mfn-opts'),
          'std' => 'About <span class="hide-mobile">Cookies</span>',
          'condition' => array( 'id' => 'gdpr2', 'opt' => 'isnt', 'val' => '' ), // is or isnt and value
        ),
      ...
      'fields' => array(
		...
		$sections['performance-general'] = array(
			...
			'fields' => array(
				...
				array(
					'id' => 'performance-image-size-disable',
					'type' => 'checkbox',
					'title' => __('Image sizes', 'mfn-opts'),
					'desc' => __('Select only image sizes you need. The remaining sizes will not be generated. You can generate them later using <a href="admin.php?page=be-tools">Regenerate Thumbnails</a>.', 'mfn-opts'),
					'options' => array(
						'blog-portfolio' => __('Blog & Portfolio Archive<span>The size is set in the Blog & Portfolio > Featured Image tab. Default: 960x750</span>', 'mfn-opts'),
						'blog-single' => __('Blog & Portfolio Single<span>The size is set in the Blog & Portfolio > Featured Image tab. Default: 1200x480</span>', 'mfn-opts'),
						'be_clients' => __('Clients 150x75<span>Clients and Clients Slider elements</span>', 'mfn-opts'),
						'portfolio-list' => __('Portfolio style: List 1920x750<span>Portfolio style: List - Portfolio Archive and Portfolio element</span>', 'mfn-opts'),
						'portfolio-mf' => __('Portfolio style: Masonry Flat 1280x1000<span>Portfolio style: Masonry Flat - Portfolio Archive and Portfolio element</span>', 'mfn-opts'),
						'portfolio-mf-w' => __('Portfolio style: Masonry Flat 1280x500<span>Portfolio style: Masonry Flat (wide images) - Portfolio Archive and Portfolio element</span>', 'mfn-opts'),
						'portfolio-mf-t' => __('Portfolio style: Masonry Flat 768x1200<span>Portfolio style: Masonry Flat (tall images) - Portfolio Archive and Portfolio element</span>', 'mfn-opts'),
						'slider-content' => __('Slider 1630x860<span>Slider element</span>', 'mfn-opts'),
						'be_thumbnail' => __('Thumbnail 150x150<span>Sticky Post Navigation, Offer Slider and Our Team Slider elements</span>', 'mfn-opts'),
					),
					'invert' => true,
				),

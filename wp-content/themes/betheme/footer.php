...
<?php
	...
	// side slide menu
	if ( mfn_opts_get('responsive-mobile-menu') && empty($mfn_global['header']) ) {
	...
	// // gdpr 2.0 // FIX: added to wp_footer action, elementor templates support
...
if( empty($_GET['visual']) ){
	...
	$mfn_view_sidebars = get_post_meta(mfn_ID(), 'mfn-template-sidemenu', true);

	if( !empty($mfn_view_sidebars) ) {

		$mfn_header_sidemenus = json_decode($mfn_view_sidebars, true);

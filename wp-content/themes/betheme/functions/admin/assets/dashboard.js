(function($) {
  ...
  var MfnDashboard = (function($) {
    ...
    var registration = {
      ...
      // registration.checkLicense()

      checkLicense: function( $el ){
        ...
        $.ajax({
          ...
          data : {
            'mfn-builder-nonce': $('input[name="mfn-builder-nonce"]').val(),
            action: 'mfn_setup_check_license',
          },
          dataType: 'JSON',
          type: 'POST',
      ...
      newLicense: function( $el ){
        ...
        $el.siblings('.toggle-content').slideToggle();
    ...
    var bind = function() {
      ...
      $dashboard.on( 'click', '.check-status', function(e) {
        e.preventDefault();
        registration.checkLicense($(this));
      });

      $dashboard.on( 'click', '.new-license .toggle-header', function(e) {

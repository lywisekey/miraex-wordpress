...
class Mfn_Builder_Helper {
	...
	public static function get_post_types( $params = array() ) {
		$args = array(
	   //'_builtin' => true
			'publicly_queryable' => true
		);

		$exclude = array('template', 'e-floating-buttons', 'elementor_library', 'icons', 'layout', 'patterns_ai_data', 'product_variation', 'shop_coupon', 'shop_order', 'shop_order_placehold', 'shop_order_refund', 'wpcf7_contact_form', 'acf-field', 'acf-field-group', 'acf-post-type', 'acf-taxonomy', 'revision', 'nav_menu_item', 'custom_css', 'customize_changeset', 'oembed_cache', 'user_request', 'wp_block', 'wp_template', 'wp_template_part', 'wp_global_styles', 'wp_navigation', 'wp_font_family', 'wp_font_face', 'attachment');
		...
		if( !empty($params['skip_mfn_cpt']) ) {
			$exclude[] = 'portfolio';
			$exclude[] = 'product';
			$include_pages = false;
			unset($params['skip_mfn_cpt']);
		}
		...
		if( !isset( $post_types['page'] ) && $include_pages ) $post_types['page'] = get_post_type_object( 'page' );
		if( !isset( $post_types['client'] ) && $include_pages ) $post_types['client'] = get_post_type_object( 'client' );
		if( !isset( $post_types['offer'] ) && $include_pages ) $post_types['offer'] = get_post_type_object( 'offer' );
		if( !isset( $post_types['slide'] ) && $include_pages ) $post_types['slide'] = get_post_type_object( 'slide' );
		if( !isset( $post_types['testimonial'] ) && $include_pages ) $post_types['testimonial'] = get_post_type_object( 'testimonial' );

		foreach ($post_types as $post_type) {
			if( is_object($post_type) && !in_array($post_type->name, $exclude) ){
				$return[] = $post_type;
			}
		}
		...
		foreach( $return as $r ) {
			...
			$taxs = get_object_taxonomies($r->name, 'objects');

			foreach ($excludes as $ex) {
				unset($taxs[$ex]);
			}

			if( !empty($r) ) $r->mfn_tax = (array) $taxs;
	...
	public static function get_post_types_tax( $params = array() ) {

		global $wpdb;

		$excludes = array('post_format', 'product_type', 'product_visibility', 'product_shipping_class', 'elementor_library_category', 'elementor_library_type', 'link_category', 'nav_menu', 'wp_pattern_category', 'wp_template_part_area', 'wp_theme', 'term_translations', 'term_language', 'post_translations', 'language');
	...
	public static function get_postmeta( $post_type = false ) {
		global $wpdb;

		// AND pm.meta_key NOT LIKE '\_%'

		/*
		AND pm.meta_key NOT LIKE '\mfn_%'
		AND pm.meta_key NOT LIKE '\mfn-%'
		*/
		...
		WHERE p.post_type = %s
			...
			AND pm.meta_key NOT LIKE '\_wp_page_template%'
			AND pm.meta_key NOT LIKE '%_template'
		  AND pm.meta_key NOT LIKE '_edit_lock'
		  AND pm.meta_key NOT LIKE '_edit_last'
		  AND pm.meta_key NOT LIKE '\_yoast_%'
		  AND pm.meta_key NOT LIKE 'mfn-page-items-backup'
		  AND pm.meta_key NOT LIKE 'mfn-page-items'
		  AND pm.meta_key NOT LIKE 'mfn-page-items-seo'
		  AND pm.meta_key NOT LIKE 'mfn-page-local-style'
		  AND pm.meta_key != ''
  ...
  public static function wrapTools($wrap = false){
  	...
  	$html = '<a href="#" class="btn-item-add mfn-item-add mfn-icon-add-light mfn-wrap-add-item" data-tooltip="Add element">Add element</a><div class="wrap-header mfn-header mfn-header-grey">

  	<div class="mfn-option-dropdown parent-selector">
			<a title="Select parent" href="#" class="mfn-option-btn mfn-option-grey parent-section-select"><span class="mfn-icon mfn-icon-selector-parent"></span></a>
			<div class="dropdown-wrapper">
				<a class="mfn-dropdown-item parent-section-select" href="#"><span class="mfn-icon mfn-icon-selector-child"></span> Section</a>
				<a class="mfn-dropdown-item parent-wrap-select" href="#"><span class="mfn-icon mfn-icon-selector-child"></span> Wrap</a>
			</div>
		</div>

  	<a class="mfn-option-btn mfn-option-blue mfn-element-menu mfn-element-edit" href="#" data-tooltip="Edit wrap" data-position="right"><span class="mfn-icon mfn-icon-wrap"></span></a>';
